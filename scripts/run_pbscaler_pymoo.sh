#!/usr/bin/env bash
# Run the PBScaler experiment with pymoo-based GA end-to-end.
# Usage: bash scripts/run_pbscaler_pymoo.sh
set -euo pipefail
...
OUT_DIR="${PROJECT_ROOT}/results/pbscaler_pymoo"
...
# ── Step 16: Comprehensive log analysis ──────────────────────────────────────
echo "==> Step 16: PBScaler log analysis"
ANALYSIS="${OUT_DIR}/analysis.txt"
LOG="${OUT_DIR}/pbscaler.log"

{
    echo "===== PBScaler pymoo Experiment Analysis ====="
    echo "Generated: $(date)"
    echo ""

    # (1) Log prefix counts
    echo "--- (1) Log Prefix Counts ---"
    if [[ -f "${LOG}" ]]; then
        for prefix in INIT ANOMALY PAGERANK GA_OPT GA_INIT GA_EVOLVE WASTE SCALE; do
            count=$(grep -c "${prefix}:" "${LOG}" 2>/dev/null || echo 0)
            echo "  ${prefix}: ${count}"
        done
    else
    ...
    # (2) ERROR / WARNING lines
    echo "--- (2) ERROR / WARNING Lines ---"
    if [[ -f "${LOG}" ]]; then
        if grep -E '(ERROR|WARNING)' "${LOG}" 2>/dev/null; then
            :
        else
            echo "  (none)"
        fi
    ...
    # (3) First complete cycle (ANOMALY → SCALE)
    echo "--- (3) First Complete Cycle (ANOMALY → SCALE) ---"
    if [[ -f "${LOG}" ]]; then
        cycle=$(awk '/ANOMALY:/{found=1} found{print} /SCALE:/{if(found) exit}' "${LOG}" 2>/dev/null)
        if [[ -n "${cycle}" ]]; then
            echo "${cycle}"
        else
            echo "  (no complete ANOMALY→SCALE cycle found)"
        fi
    else
        echo "  (no log file found)"
    fi
    ...
    # (4) Replica count chart
    echo "--- (4) Replica Count Chart ---"
    CHART="${OUT_DIR}/replica_count_over_time.png"
    if [[ -f "${CHART}" ]]; then
        echo "  OK: ${CHART}"
    else
        echo "  NOT FOUND: ${CHART}"
    fi
