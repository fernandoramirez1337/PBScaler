...
class PBScaler:
    ...
    def waste_detection(self):
        ...
        self.roots = self._filter_waste_candidates(waste_mss)
        logger.info(f'WASTE: Waste roots (after filter): {self.roots}')
        if len(self.roots) != 0:
            self.choose_action(option='reduce')

    def _filter_waste_candidates(self, waste_mss):
        """Filter scale-down candidates. Subclasses override to use k_eff. Cap_3 sec:scaledown."""
        return list(filter(lambda ms: self.svc_counts[ms] > self.min_num, waste_mss))
